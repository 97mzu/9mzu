...
local function Draggable(Bar, Window, enableTaptic, tapticOffset)
    pcall(function()
        ...
        -- Removido: não há mais função connectFunctions para dragBar
        ...
        UserInputService.InputChanged:Connect(function(Input)
            if Input == DragInput and Dragging then
                ...
                -- Removido: não atualiza mais dragBar
            end
        end)
    end)
end
...
if newNotification:FindFirstChild("Logo") then
...
-- mobile size
    MainSize = UDim2.fromOffset(550, 365)
    MinSize = UDim2.fromOffset(400, 42)
end
...
function Luna:CreateWindow(WindowSettings)
	WindowSettings = Kwargify({
		Name = "9mzu Hub",
		Subtitle = "discord.gg/vmDG4bHUUv",
		LogoID = "6031097225",
	...
	WindowSettings.KeySettings.SecondAction = Kwargify({
		...
		Parameter = "https://discord.gg/YfHrnbDqhh" -- for discord, add the invite link like home tab. for link, type the link of ur key sys
	...
	if WindowSettings.KeySystem then
		...
		if dragBar then Draggable(dragInteract, Main, true, 0) end
		...
		if not Passthrough then
			...
			Btn.Interact.MouseButton1Click:Connect(function()
				if typesys == "Discord" then
					setclipboard(tostring("https://discord.gg/YfHrnbDqhh"..KeySettings.SecondAction.Parameter)) -- Hunter if you see this I added copy also was too lazy to send u msg
					if request then
						request({
	...
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0, Size = MainSize}):Play()
	...
	TweenService:Create(Main.Title.Title, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.1}):Play()
	TweenService:Create(Main.Title.subtitle, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.1}):Play()
	...
	TweenService:Create(Navigation.Player.icon.UIStroke, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Transparency = 1}):Play()
	TweenService:Create(Main.Line, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	task.wait(0.4)
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			Icon = 1,
			SupportedExecutors = {"Vega X", "Delta", "Nihon", "Xeno", "Solara"}, -- THESE DEFAULTS ARE PLACEHOLDERS!! I DO NOT ADVERTISE THESE, THEY ARE JUS THE FIRST THAT CAME TO MIND. I HAVE NO IDEA WHETHER THEYA RE RATS (they prob are) AND IM NOT RESPONSIBLE IF U GET VIRUSES FROM INSTALLING AFTER SEEING THIS LIST
			DiscordInvite = "https://discord.gg/YfHrnbDqhh" -- The disvord invite link. Do not include the link so for example if my invite was discord.gg/nebula I would put nebula
		}, HomeTabSettings or {})
		...
		function HomeTab:Activate()
			...
			tween(HomeTabButton.UIStroke, {Transparency = 1})
...
HomeTabPage.player.Text.Text = "Welcome, " .. displayName

if playerName == "xmanu29x" then
    HomeTabPage.player.user.Text = playerName ..
        " - <font color='#FF4B4B'>Co-Owner,</font> <font color='#A020F0'>Developer,</font> <font color='#0057FF'>Goat,</font> <font color='#008214'>Special Player</font>"
elseif playerName == "eri77ll" or playerName == "Eternalvra" or playerName == "9mzuy" or playerName == "notok3dkap" then
    HomeTabPage.player.user.Text = playerName ..
        " - <font color='#FF4B4B'>Owner,</font> <font color='#A020F0'>Developer,</font> <font color='#0057FF'>Goat,</font> <font color='#008214'>Special Player</font>"
else
    HomeTabPage.player.user.Text = playerName
end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:Activate()
			...
			tween(TabButton.UIStroke, {Transparency = 1})
    ...
    if bool then
        tween(Toggle.toggle, {BackgroundColor3 = Color3.fromRGB(87, 164, 255), BackgroundTransparency = 0})
        ...
        tween(Toggle.toggle, {BackgroundColor3 = Color3.fromRGB(97,97,97), BackgroundTransparency = 0})
	...
	tween(Navigation.Line, {BackgroundTransparency = 1})
...
Navigation.Size = UDim2.new(Navigation.Size.X.Scale, 205, Navigation.Size.Y.Scale, Navigation.Size.Y.Offset)
Elements.Parent.Size = UDim2.new(1, -205, Elements.Parent.Size.Y.Scale, Elements.Parent.Size.Y.Offset)
